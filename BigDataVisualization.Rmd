...
In this tutorial, we use sparklyr and are limited to using it to summarize a larger data set for the purpose of creating visualizations, which does get us pretty far.
...
airlines <- readr::read_csv(file.path('data', 'flights2016.csv.gz'))
...
This is an interactive faceted display that opens in your web browser. There are 140 routes out of Atlanta that fit our criteria. 140 panels is too many to display at once, which is why the display is showing the first 8, by default ordered alphabetically by our grouping variables, origin and destination airport code.
...
One piece of data we can use to augment our display is metadata about the airports. The `nycflights13` R package contains such a data set.
...
Since we only are looking at a single origin airport, let's add airport metadata for our destinations. Let's select a few variables from this data set and rename the variables to be more meaningful for being applied to destination airports.
...
***
**Your Turn:** Investigate the Trelliscope display and try to answer the following questions:
- Can you turn on the `airport_name` label to give you a helpful hint as to where or what the destination airport code is referring to?
